...
caminho = os.path.join(
    base_dir, '..', '..', 'data', 'output', 'dados_ficticios.csv'
)
...
def create_coluna_churn(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'ATIVO/EVADIDO' not in df.columns:
        raise KeyError(
            "A coluna 'ATIVO/EVADIDO' não foi encontrada no DataFrame."
        )
...
if __name__ == '__main__':
    print(
        f" - Proporção de churn (evadidos):{round(dados['CHURN'].mean() * 100, 3)}%"
    )
